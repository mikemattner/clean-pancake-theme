...
<?php if (count($invoices)): ?>
	...
	<table id="kitchen-invoices"  class="kitchen-table" cellpadding="0" cellspacing="0">
		...
		<?php foreach ($invoices as $invoice): ?>
			...
			<td><?php echo ($invoice->paid ? 'Paid' : 'Unpaid'); ?></td>
			<td>
	...
	<?php foreach ($projects as $project): ?>
		...
		<p style="font-size:12px; color:#ccc"><?php echo lang('projects:due_date') ?>: <?php echo date('m/d/Y', $project->due_date); ?><br>
		...
		<table id="kitchen-projects" class="kitchen-table" cellpadding="0" cellspacing="0">
			<?php foreach ($project->tasks as $task): ?>
				...
				<tr class="items-desc-row">
					...
					<td><?php echo $task['due_date'] ? date('m/d/Y', $task['due_date']) : 'N/A'; ?></td>
					<td>
	...
	<table id="kitchen-proposals" class="kitchen-table" cellpadding="0" cellspacing="0">
		...
		<?php foreach ($proposals as $proposal): ?>
			...
			<td><?php echo __('proposals:' . (!empty($proposal->status) ? strtolower($proposal->status) : 'noanswer'), array(date($this->settings->get('date_format'), $proposal->last_status_change))); ?></td>
			<td>
