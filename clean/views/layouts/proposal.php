	...
	<div id="buttonBar" data-status="<?php echo $proposal['status'];?>" >
        ...
        <?php if (is_admin()): ?>
            ...
            <?php echo anchor(Settings::get('kitchen_route').'/'.$proposal['client']['unique_id'], 'Go to Client Area &rarr;', 'class="button"'); ?>
                    ...
                    <?php echo anchor('admin', ($proposal['is_viewable'] ? 'Viewable in client area' : 'Not viewable in client area'), 'class="toggle_is_viewable '.($proposal['is_viewable'] ? 'viewable' : 'not-viewable').' button" data-not-viewable="Not viewable in client area" data-viewable="Viewable in client area"'); ?>
...
<?php endif; ?>
        <div class="sidebar">
             ...
             <div class="clientEdit">
                <h2><?php echo lang('global:client') ?></h2>
                <select class="clientDetails">
                    <?php foreach ($clients as $client) : ?>
                    <option data-clientid="<?php echo $client['id'];?>" data-company="<?php echo $client['company'];?>" data-first-name="<?php echo $client['first_name'];?>" data-last-name="<?php echo $client['last_name'];?>" data-address="<?php echo nl2br($client['address']);?>"><?php echo $client['title'];?></option>
                    <?php endforeach; ?>
                </select>
             </div>
